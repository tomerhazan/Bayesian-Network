...
public class Algorithm3 {
    ...
    public double calcAlgo(Var[] Variables, Query queryy) {
        ...
        } else if (checkParentsWithEvidence()) {
    ...
    // STEP 1: BUILD HEURISTIC LIST (I TALK ABOUT IT IN THE DETAILS.DOCX FILE)
    // STEP 2 LIKE ALGO 1
    ...
    private void joinFactor(boolean bool) {
        ...
        String[] best = findBest(sortedHidden);
    ...
    private int countMultiply(String name){
        ...
        HashMap<String,HashMap> finals = new HashMap<>();
        String[] list = new String[joinByName.size()];
        int index = 0;
        for(String factorKey: joinByName.keySet()){
            HashMap<HashMap<String,String>,String> Map = joinByName.get(factorKey);
            HashMap<String,Integer> factorsValues = new HashMap<>();
            for(Object key: Map.keySet()){
                HashMap<String,String> line = (HashMap<String, String>) key;
                for (String keyName: line.keySet()){
                    for(int i = 0; i<variables.length;i++){
                        if(keyName.equals(variables[i].Name)){
                            factorsValues.put(keyName,variables[i].Values.length);
                            break;
                        }
                    }
                }
            ...
            finals.put(factorKey,factorsValues);
            list[index++] = factorKey;
        }

        while (list.length > 1){
            HashMap<String,Integer> combineAllOptions = new HashMap<>();
            for (int i = 0; i < list.length-1; i++) {
                for (int j = i+1; j < list.length; j++) {
                    String Name = list[i] + " join " + list[j];
                    int answer = calc2Values(finals.get(list[i]),finals.get(list[j]));
                    combineAllOptions.put(Name,answer);
                }
            }
            int in = 0;
            String n = "";
            for(String key: combineAllOptions.keySet()){
                if (in == 0){
                    in = combineAllOptions.get(key);
                    n = key;
                }else{
                    if(combineAllOptions.get(key) < in){
                        in = combineAllOptions.get(key);
                        n = key;
                    }
                }
            ...
            ans *= in;
            String[] split = n.split(" join ");
            String[] newList = new String[list.length-2];
            if (newList.length > 0){
                int inde = 0;
                for (int i = 0; i < list.length; i++) {
                    if(!(list[i].equals(split[0]) || list[i].equals(split[1]))){
                        newList[inde++] = list[i];
                    }
                }
                list = newList;
    ...
    private Integer calc2Values(HashMap first, HashMap second){
        int ans = 1;

        for(Object key: first.keySet()){
            int a = (int) first.get(key);
            ans *= a;
        }

        for(Object key: second.keySet()){
            if(!first.containsKey(key)){
                int a = (int) second.get(key);
                ans *= a;
            }
        }
        return ans;
    ...
    private String[] findBest(TreeMap<String, String> hidden){
        String[] best = new String[hiddenVars.length];
        HashMap<String,Integer> bestMap = new HashMap<>();
        for(String key: hidden.keySet()){
            String name = key;
            int countMult = countMultiply(key);
            bestMap.put(name,countMult);
        }
        int index = 0;
        while(bestMap.size() > 0){
            int lower = 0;
            String nam = "";
            for(String key: bestMap.keySet()){
                if (lower == 0){
                    nam = key;
                    lower = bestMap.get(key);
                }else{
                    if(bestMap.get(key) < lower){
                        lower = bestMap.get(key);
                        nam = key;
                    }
                }
    ...
    private HashMap<HashMap<String, String>, String> findFactor(HashMap factor1, HashMap factor2,String name){
        ...
        for (Object factor2Key: factor2.keySet()){
            ...
            for(String factor2LinesKey: factor2Lines.keySet()){
                ...
                // check if the var(name) is exist in the factor columns, if no - add it to the new factor.

                if(!checkIfVarIsInTheFactor(newFactor,factor2LinesKey)){

                    // get all the values that the var have.
                    String[] varValuesToAdd = getVarValuesByName(factor2LinesKey);

                    // add the var values to the factor
